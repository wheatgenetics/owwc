...
#SBATCH --job-name=hisat2.wgrc.gbs.258
...
#SBATCH --array=1-258               # 1-4 or 1,2,3-4 or stepwise 1-5:2 for 1,3,5
...
## WGS data
	seq_files="/bulk/lianggao/owwc/data/raw.demulti.gbs.wgrc.258"
...
## aligning reads to ref genome using bowtie2 mem
	...
	$hisat2 -p "${cores}" -x "${dbPath}" -U "${forward}" | $samtools view -@ "${cores}" -bhS - > bam/"${sample}".bam  ## note: cannot use -f 2
